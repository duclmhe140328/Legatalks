...
function MeetingSystemMessage({ message }) {
  ...
  return <div className="meeting-message-event">
    ...
    <div><b>{meeting.title || 'Phòng họp Nexora'}</b><span>{startsAt ? `Bắt đầu ${format(startsAt, 'HH:mm dd/MM/yyyy')}` : 'Có thể vào phòng ngay'} · {meeting.durationMinutes || 60} phút</span></div>
...
function SharedPostMessage({ post }) {
  ...
  return <a className="shared-post-message" href={`/timeline?post=${post.postId}`}>
    ...
    <div className="shared-post-copy"><span><Newspaper size={14} /> Bài viết được chia sẻ</span><b>{post.author?.displayName || 'Người dùng Nexora'}</b><p>{post.text?.slice(0, 180) || (post.contentType === 'video' ? 'Video' : 'Mở bài viết')}</p></div>
  </a>;
}
