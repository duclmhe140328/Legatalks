...
export default function JitsiMeetFrame({
  ...
  const title = firstNonEmpty(meeting?.title, subject, 'Nexora Meeting');

  const dispose = () => {
    try {
      apiRef.current?.dispose?.();
    } catch {
      // IFrame API may already be disposed after readyToClose.
    }
    apiRef.current = null;
  ...
  useEffect(() => {
    ...
    async function mountJitsi() {
      try {
        ...
        if (cancelled || !parentRef.current || !window.JitsiMeetExternalAPI) {
          return;
        }
        ...
        const instance = new window.JitsiMeetExternalAPI(
          domainFromUrl(effectiveServer),
          {
            ...
            configOverwrite: {
              ...
              toolbarButtons:
                Array.isArray(configOverwrite.toolbarButtons) &&
                configOverwrite.toolbarButtons.length
                  ? configOverwrite.toolbarButtons
                  : Array.isArray(backendConfig.toolbarButtons) &&
                      backendConfig.toolbarButtons.length
                    ? [
                        ...new Set([
                          ...backendConfig.toolbarButtons,
