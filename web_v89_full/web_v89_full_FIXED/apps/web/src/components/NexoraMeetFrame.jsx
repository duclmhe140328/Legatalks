...
export default function NexoraMeetFrame({ meetingId, onEnded, onMinimize, onExpand, onClosed, compact = false }) {
  ...
  const [status, setStatus] = useState('Đang tải phòng họp Nexora RTC…');
  ...
  useEffect(() => {
    ...
    setClosed(false);
    setStatus('Đang tải phòng họp Nexora RTC…');
  ...
  return <section className={`nexora-meet-stage card ${compact ? 'is-compact' : ''}`}>
    <div className="jitsi-stage-head">
      <div><b>{meeting?.title || 'Phòng họp Nexora'}</b><span>{compact ? 'Đang họp ở cửa sổ nhỏ' : 'Nexora RTC · web/app dùng chung · có share màn hình mobile'}</span></div>
