            ...
            return (
              <article key={call._id} className={`call-history-item status-${call.status}`}>
                ...
                <div className="call-history-copy">
                  <div><b>{call.conversation?.type === 'group' ? call.conversation.name : peer?.displayName || 'Người dùng Legatalk'}</b><span className={`call-status status-${call.status}`}>{statusLabels[call.status] || call.status}</span></div>
